...
void Game::init() {
    InitWindow(0, 0, "Привет");

    this->windowX = GetScreenWidth();
    this->windowY = GetScreenHeight();
    SetWindowSize(this->windowX, this->windowY);
    SetConfigFlags(FLAG_FULLSCREEN_MODE | FLAG_VSYNC_HINT);
    SetTargetFPS(60);
    this->loadResource();
    this->currScene = std::make_unique<MainMenuScene>();
...
void Game::loadResource(){
    int count = 0;
    int* codepoints = getCompleteCodepoints(&count);
    this->font = LoadFontEx("roboto/Roboto-Regular.ttf", 70, codepoints, count);
    delete[] codepoints;
...
void Game::run(){
    ...
    while(!WindowShouldClose()){
        ...
        if(this->currScene.get()){
            this->currScene->update(this, dt);
            this->currScene->draw(this);
        }
...
void Game::changeScene(GameState state) {
    switch(state){
        case GameState::MAIN_MENU:
            this->currScene.reset(new MainMenuScene);
            break;
        case GameState::IN_GAME:
            this->currScene.reset(new GameScene);
            break;
    }
}